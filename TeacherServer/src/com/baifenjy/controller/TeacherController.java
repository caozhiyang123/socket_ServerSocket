        ...
        public void run()
        {
            try {
                while (connected) {
                    ...
                    if (Request.COMMIT_TEC.equals(request)) {
                    ...
                    } else if (Request.QUERY_TEC_BY_ORDERID.equals(request)) {
                        String orderId = dis.readUTF();
                        List<Teacher> tecs = teacherService.queryByOrderId(orderId);
                        String tecsString = JSON.toJSONString(tecs.size()==0?null:tecs);
                        dos.writeUTF(tecsString);
                    } else if(Request.QUERY_TEC_BY_TEACHER_PHONE.equals(request)){
                    ...
                    if(Request.BOUND_ORDER.equals(request)){
                        String orderId = dis.readUTF();
                        String teacherId = dis.readUTF();
                        boolean flag = teacherService.boundOrder(orderId,teacherId);
                        if(flag){
                            dos.writeUTF(Response.SUCCESS);
                        }else{
                            dos.writeUTF(Response.FAIL);
                        }
                    }else if(Request.RELEASE_ORDER.equals(request)){
                        String orderId = dis.readUTF();
                        String teacherId = dis.readUTF();
                        boolean flag = teacherService.releaseOrder(orderId,teacherId);
                        if(flag){
                            dos.writeUTF(Response.SUCCESS);
                        }else{
                            dos.writeUTF(Response.FAIL);
